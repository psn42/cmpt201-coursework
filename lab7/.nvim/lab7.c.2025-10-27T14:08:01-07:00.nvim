...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  for (int i = 0; i < *result_count; i++) {
    if (output[i].doubled_value == input[i].doubled_value) {
  ...
  output[*result_count - 1].doubled_value = input->doubled_value;
  output[*result_count - 1].line_numbers[0] = input->line_number;
